...
export const Route = createFileRoute('/dashboard/vessels/search' as any)({
...
function VesselsSearch() {
  ...
  const search = useSearch({ from: '/dashboard/vessels/search' as any }) as { q: string }
  ...
  const onSubmit = (e: React.FormEvent) => {
    e.preventDefault()
    navigate({ to: '/dashboard/vessels/search' as any, search: { q: input } })
  }
  ...
  const columns = useMemo(
    () => [
      helper.accessor('entity_id', {
        header: 'Entity',
        cell: (info) => (
          <a className="underline" href={`/dashboard/vessels/entity/${encodeURIComponent(info.getValue())}`}>{info.getValue()}</a>
        ),
      }),
