...
This guide described the former Label Studio setup. Label Studio is no longer part of the Oceanid stack. For current setup and access patterns, see the Networking Architecture and GPU Access sections in the repository README.
...
| Where are my files? | Ops can provide the exact path | Default: `raw-uploads/<project-slug>-<id>/import/` for uploads, `annotations/<project-slug>-<id>/export/` for labeled data | Paths are auto-created; ask Ops if you need a custom location |
...
- Default folder structure (unless Ops overrides it):
  - Source files: `raw-uploads/<project-slug>-<project-id>/import/`
  - Annotations: `annotations/<project-slug>-<project-id>/export/`
...
**Where in S3 are my files?**

- By default, uploads go to `raw-uploads/<project-slug>-<project-id>/import/` and completed annotations to `annotations/<project-slug>-<project-id>/export/`.
- The `<project-slug>` is based on the project title (lowercase, hyphenated) plus the internal numeric id.
- If you need a custom folder layout, let Ops know and we’ll update the configuration.
